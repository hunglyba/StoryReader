...
class DetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate,FBNativeAdDelegate {
...
//        print("local chapter detail \(array)")
    ...
    {
        self.headerFbAd = HeaderFbAd.instanceFromNib()
        self.headerFbAd.titleLabel.text = nativeAd.title
        self.headerFbAd.detailLabel.text = nativeAd.subtitle
        self.headerFbAd.buttonInstall.setTitle(nativeAd.callToAction, forState: UIControlState(rawValue: 0))
        self.headerFbAd.buttonInstall.backgroundColor = UIColor.greenSeaColor()
        self.headerFbAd.buttonInstall.setTitleColor(UIColor.whiteColor(), forState: UIControlState(rawValue: 0))
        self.headerFbAd.imageIconView.kf_setImageWithURL((nativeAd.icon?.url)!,
...
//        }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            {
                let cell: FbAdCell = tableView.dequeueReusableCellWithIdentifier("AdCell") as! FbAdCell
                
                cell.setFacebookAd(self.nativeAd)
                self.nativeAd.registerViewForInteraction(cell, withViewController: self)
